...
# Dump actual test code to a MiniAOD_${SampleName}_test.sh file that can be run in Singularity
cat << EndOfTestFile > MiniAOD_${SampleName}_test.sh
#!/bin/bash

export SCRAM_ARCH=el8_amd64_gcc12

source /cvmfs/cms.cern.ch/cmsset_default.sh
echo "Running MiniAOD_${SampleName}_test.sh "
echo "pwd: "
pwd
echo "ls -ltrh: "
ls -ltrh 
...
# cmsDriver command
cmsDriver.py  --era Run3_2024 --customise Configuration/DataProcessing/Utils.addMonitoring --step PAT --geometry DB:Extended --conditions 150X_mcRun3_2024_realistic_v2 --datatier MINIAODSIM --eventcontent MINIAODSIM1 --python_filename MiniAOD_${SampleName}_1_cfg.py --fileout file:${outputFile} --filein file:${inputFile}  --number \${EVENTS} --number_out \${EVENTS} --no_exec --mc || exit \$? ;
...
REPORT_NAME=MiniAOD_${SampleName}_report.xml
# Run the cmsRun
cmsRun -e -j \$REPORT_NAME MiniAOD_${SampleName}_1_cfg.py || exit \$? ;

printf "\n\ncmsRun: done \n\n"
# Parse values from MiniAOD_${SampleName}_report.xml report
...
echo "Validation report of MiniAOD_${SampleName} sequence 1/1"
...
# End of MiniAOD_${SampleName}_test.sh file
EndOfTestFile

printf "\n\n cat MiniAOD_${SampleName}_test.sh  \n"
cat MiniAOD_${SampleName}_test.sh
printf "\n\n"

# Make file executable
chmod +x MiniAOD_${SampleName}_test.sh
...
singularity run --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/MiniAOD_${SampleName}_test.sh)
