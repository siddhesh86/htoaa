...
for X4bTaggerRegion in ['SR', 'SB']:
    ...
    selectionTags_dict[X4bTaggerRegion] = selectionTag
...
for subCat in subCats:
    for selectionTagNameShort, selectionTagName in selectionTags_dict.items():
        for histgramName0, histogramName_dict in histogramNames_dict.items():
            ...
            for trgSelectionNameShort, trgSelectionName in triggerSelections_dict.items():
                selectionTag_toUse = '%s%s_%s' % (subCat,trgSelectionName, selectionTagName)
                histograms_dict[histgramName][sHistosToOverlay][trgSelectionNameShort] = []
                    ...
                    for dataset in MCBkg_list:
                        ...
                        for sDatasetName, YearsToRun_list in YearsToRun_dict.items():
                            for Year_ in YearsToRun_list:
                                histograms_dict[histgramName][sHistosToOverlay][trgSelectionNameShort].append({
