...
from htoaa_CommonTools import (
    ...
    getPURewgts_variation, get_jetTriggerSF, get_metTriggerSF, 
    get_PSWeight, add_pdf_as_weight, get_QCDScaleWeight, add_HiggsEW_kFactors,
    get_JER_and_JES,
...
class ObjectSelection:
    def __init__(self, era):
        ...
        self.METPt_ZvvLo_MinThsh   = 200 # 250 # 300
        self.METPt_ZvvLo_MaxThsh   = 300
        self.METPt_ZvvHi_MinThsh   = 300 # 250 # 300
        self.METPt_ZvvHi_MaxThsh   = 999999.0
...
class HToAATo4bProcessor(processor.ProcessorABC):
    def __init__(self, datasetInfo={}):
        ...
        global runMode_2018HEM1516IssueValidation; runMode_2018HEM1516IssueValidation = True # False
        ...
        sTrgSelection = self.datasetInfo['triggers'] if self.datasetInfo['triggers'] else 'Trg_Combo_MET'
        ...
        categories_dict = OD()
        categories_dict["ZvvIncl"] = [ "METPt_ZvvIncl" if s_ == "METPt" else s_     for s_ in self.sel_names_all["Presel"] ]
        ...
        #for iCondition in range(self.sel_names_all["Presel"].index("candH"), len(self.sel_names_all["Presel"]) - 1):
        ...
        pt2TeV_axis           = hist.Bin("Pt2TeV",                 r"$p_{T}$ [GeV]",             220,    -200,    2000)
            ...
            for sHExt_0 in self.histosExtensions:    
                ...
                if runMode_2018HEM1516IssueValidation:
                    histos.update(OD([
                        ...
                        # MET
                        ('hMETPt_DataPreHEM1516Issue'+sHExt,                          {sXaxis: pt_axis,         sXaxisLabel: r"$p_{T}(leading FatJet)$ [GeV]"}),
                        ('hMETEta_DataPreHEM1516Issue'+sHExt,                         {sXaxis: eta_axis,        sXaxisLabel: r"\eta (leading FatJet)"}),
                        ('hMETPhi_DataPreHEM1516Issue'+sHExt,                         {sXaxis: phi_axis,        sXaxisLabel: r"\phi (leading FatJet)"}),
                        ('hMETPt_DataWithHEM1516Issue'+sHExt,                          {sXaxis: pt_axis,         sXaxisLabel: r"$p_{T}(leading FatJet)$ [GeV]"}),
                        ('hMETEta_DataWithHEM1516Issue'+sHExt,                         {sXaxis: eta_axis,        sXaxisLabel: r"\eta (leading FatJet)"}),
                        ('hMETPhi_DataWithHEM1516Issue'+sHExt,                         {sXaxis: phi_axis,        sXaxisLabel: r"\phi (leading FatJet)"}),
    ...
    def process_shift(self, events, shift_syst=None):
        ...
        if 'particleNetMD_Hto4b_Haa4b' in events.FatJet.fields:
            ...
            leadingFatJet_PNet_massA_Hto4b_avg = calculateAverageOfArrays([
                ak.fill_none(leadingFatJet.particleNet_massA_Hto4b_v0, 0),
                ak.fill_none(leadingFatJet.particleNet_massA_Hto4b_v1, 0),
                ak.fill_none(leadingFatJet.particleNet_massA_Hto4b_v3, 0)
            ])
        ...
        if "leadingFatJetPt" in self.sel_conditions_all_list:
            ...
            selection.add(
                "leadingFatJetPt",
                leadingFatJet.pt_toUse > self.objectSelector.FatJetPt_Zvv_MinThsh
            )
        ...
        if "METPt_ZvvIncl" in self.sel_conditions_all_list:
            selection.add(
                "METPt_ZvvIncl",
                ((METToUse.pt_toUse >  self.objectSelector.METPt_ZvvIncl_MinThsh) & 
                 (METToUse.pt_toUse <= self.objectSelector.METPt_ZvvIncl_MaxThsh) )
            )
        if "METPt_ZvvLo" in self.sel_conditions_all_list:
            selection.add(
                "METPt_ZvvLo",
                ((METToUse.pt_toUse >  self.objectSelector.METPt_ZvvLo_MinThsh) & 
                 (METToUse.pt_toUse <= self.objectSelector.METPt_ZvvLo_MaxThsh) )
            )
        if "METPt_ZvvHi" in self.sel_conditions_all_list:
            selection.add(
                "METPt_ZvvHi",
                ((METToUse.pt_toUse >  self.objectSelector.METPt_ZvvHi_MinThsh) & 
                 (METToUse.pt_toUse <= self.objectSelector.METPt_ZvvHi_MaxThsh) )
            )
        ...
        if self.datasetInfo["isMC"]:
            ...
            # MC TrgEff ------------------------------------
            wgt_TrgEff, wgt_TrgEffUp, wgt_TrgEffDown  = get_metTriggerSF(
                pt   = METToUse.pt_toUse,  
                year = self.datasetInfo["era"]
            )
            ...
            if shift_syst is None:
                ...
                if (not self.datasetInfo['systematicsToRun'] == 'no') and self.datasetInfo['runSystematics']:
                    ...
                    if stringHasSubstring(self.datasetInfo['systematicsToRun'], ['trg', 'full'] ):
                        systList.extend( [
                            self.systNameMetTrigEffi+SystNameConvUp,
                            self.systNameMetTrigEffi+SystNameConvDown,
                        ] )
        ...
        if (printLevel >= 0) and (storeIndividualEvtWgts):
            ...
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=[self.systNameMetTrigEffi])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt', self.systNameMetTrigEffi])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt', self.systNameMetTrigEffi, self.systNameBtagUncorr])) / nEvents = }\n\n")
            ...
            for sel_name in self.sel_names_all.keys(): # loop of list of selections
                ...
                for sHExt_0 in self.histosExtensions: # HistogramNameExtensions_QCD = ['_0b', '_1b', '_2b', '_3b', '_4b', '_5bAndMore'], else ['']
                    ...
                    if self.datasetInfo["era"] == Era_2018 and runMode_2018HEM1516IssueValidation:
                        ...
                        output['hMETPt_DataPreHEM1516Issue'+sHExt].fill(
                            dataset=dataset,
                            Pt=(events.MET.pt[ sel_tmp_ ]),
                            systematic=syst,
                            weight=evtWeight_DataPreHEM1516Issue[ sel_tmp_ ]
                        )
                        output['hMETPhi_DataPreHEM1516Issue'+sHExt].fill(
                            dataset=dataset,
                            Phi=(events.MET.phi[ sel_tmp_ ]),
                            systematic=syst,
                            weight=evtWeight_DataPreHEM1516Issue[ sel_tmp_ ]
                        )
                        ...
                        output['hMETPt_DataWithHEM1516Issue'+sHExt].fill(
                            dataset=dataset,
                            Pt=(events.MET.pt[ sel_tmp_ ]),
                            systematic=syst,
                            weight=evtWeight_DataWithHEM1516Issue[ sel_tmp_ ]
                        )
                        output['hMETPhi_DataWithHEM1516Issue'+sHExt].fill(
                            dataset=dataset,
                            Phi=(events.MET.phi[ sel_tmp_ ]),
                            systematic=syst,
                            weight=evtWeight_DataWithHEM1516Issue[ sel_tmp_ ] 
...
if __name__ == '__main__':
    ...
    primaryDatasets     = config['primaryDatasets'] if 'primaryDatasets' in config else ['MET']
