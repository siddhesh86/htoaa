	...
	do
		...
		do
            ...
            do
                ...
                do
                    if [ $(expr $iMiniAOD_file % $MaxMiniAODFilesPerSet) -eq 0 ]; then
                        ls ${miniAOD_file} >  "miniAODs_${prod}_Pt${HiggsPtMin}_M-${mA}_${EraYear}_${iMiniAOD_file_set}.txt"
                    else
                        ls ${miniAOD_file} >> "miniAODs_${prod}_Pt${HiggsPtMin}_M-${mA}_${EraYear}_${iMiniAOD_file_set}.txt"
                    fi
                    printf "   miniAOD_file: ${miniAOD_file} ${iMiniAOD_file} : ${iMiniAOD_file_set}\n"

                    iMiniAOD_file=$((iMiniAOD_file + 1))                  
                    if [ $(expr $iMiniAOD_file % $MaxMiniAODFilesPerSet) -eq 0 ]; then
                        iMiniAOD_file_set=$((iMiniAOD_file_set + 1))
                    fi                    
                done
