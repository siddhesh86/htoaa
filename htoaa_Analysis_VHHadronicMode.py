...
from htoaa_CommonTools import (
    ...
    getPURewgts_variation, get_jetTriggerSF, get_PSWeight, add_pdf_as_weight, get_QCDScaleWeight, add_HiggsEW_kFactors,
    get_JER_and_JES,
    get_Ak4BtagSF, get_L1TPrefiringWgt,
    get_PNet_WZvsQCD_EffiSF,
    get_QCDPtWgt,
    calculatePNet_MassH_MassA_cor_and_syst,
...
class ObjectSelection:
    def __init__(self, era):
        ...
        self.wp_PNet_Xto4bv1_Htoaa4bOverQCD = 'WP-60' # 'WP-40' 'WP-80' #'WP-60'

        self.FatJetsPt_Thsh             = 250.0 # 170
        self.FatJetHiggsPt_Thsh         = 250.0
        ...
        self.VFatJetPt_VjjIncl_MinThsh = 250 #200
        self.VFatJetPt_VjjIncl_MaxThsh = 999999
        self.VFatJetPt_VjjLo_MinThsh   = 250 #200
        self.VFatJetPt_VjjLo_MaxThsh   = 400 #300
        self.VFatJetPt_VjjHi_MinThsh   = 400 #300
        self.VFatJetPt_VjjHi_MaxThsh   = 999999
        self.VFatJetMsoftdrop_Vjj_MinThsh =  50
        self.VFatJetMsoftdrop_Vjj_MaxThsh = 220
        ...
        self.TopFatJetMsoftdrop_tt0l_MinThsh =  50
        self.TopFatJetMsoftdrop_tt0l_MaxThsh = 220
        ...
        self.mH_mA_regions_unblindingChkStp3p1 = {
            ...
            'mA45to55_SR':     {'mH': [110.0, 140.], 'mA': [45.0, 55.0]},
            'mA45to55_SBmHLo': {'mH': [100.0, 110.0], 'mA': [45.0, 55.0]},
            'mA45to55_SBmHHi': {'mH': [140.0, 160.0], 'mA': [45.0, 55.0]},
            'mA45to55_SBmALo': {'mH': [110.0, 140.], 'mA': [39.0, 45.0]},
            'mA45to55_SBmAHi': {'mH': [110.0, 140.], 'mA': [55.0, 61.0]},
...
class HToAATo4bProcessor(processor.ProcessorABC):
    def __init__(self, datasetInfo={}):
        ...
        '''
        self.sel_names_all["Presel_VjjHi"] = [
            "nPV",
            "METFilters",
            "candH",
            "leadingFatJetPt",
        ...
        self.sel_names_all["Presel_VjjLo"] = [
            "nPV",
            "METFilters",
            "candH",
            "leadingFatJetPt",
        ...
        if runMode_OptimizePNetTaggerCut:
            ...
            self.sel_names_all = OD([
                ("Presel",                    [
                    ...
                    #"leadingFatJet_nSV"
                    "VFatJetPt", # leadingNonHto4bFatJet leadingNonHto4bFatJet_asSingletons
        ...
        #for iCondition in range(self.sel_names_all["Presel"].index("candVjj"), len(self.sel_names_all["Presel"]) - 1):
        #for iCondition in range(self.sel_names_all["Presel"].index("Haa4bVjjAll_Andrew"), len(self.sel_names_all["Presel"]) - 1):
        ...
        self.systNameAK8JetPNetWZTag = SystNameConvs['AK8JetPNetWZTag'].replace('$YEAR', self.datasetInfo["era"])        
            ...
            for sHExt_0 in self.histosExtensions:    
                ...
                if self.datasetInfo['histogramSaveLevel'] >= 1:
                    ...
                    if self.datasetInfo['isSignal'] and (not self.datasetInfo['isSignalGGH']) : 
                        histos.update(OD([
                            ('hEventWeight_HiggsEW'+sHExt,                        {sXaxis: Weight_axis,    sXaxisLabel: 'hEventWeight_HiggsEW'}),
                        ]))
                    if self.datasetInfo['isQCD_bGen']:
                        histos.update(OD([
                ...
                if self.datasetInfo['histogramSaveLevel'] >= 0:
                    histos.update(OD([
                        ...
                        ('hLeadingNonHto4bFatJetPhi_EtaLtm1p1'+sHExt,                   {sXaxis: phi_axis,          sXaxisLabel: r"\phi (leading NonHto4bFatJet) for eta < -1.1"}),
                        ...
                        ('hLeadingNonHto4bTopFatJetPNet_TvsQCD'+sHExt,        {sXaxis: mlScore_axis1k,    sXaxisLabel: r"LeadingNonHto4bTopFatJetPNet_TvsQCD"}),
    ...
    def process(self, events):
        ...
        if printLevel >= 10:
            ...
            print(f"\n events.FatJet.fields ({type(events.FatJet.fields)}): {events.FatJet.fields}"); sys.stdout.flush()
    ...
    def process_shift(self, events, shift_syst=None):
        ...
        if 'PNet_X4b_v2a_Haa34b_score' not in FatJetsToUse.fields or CrossCheckEvtYieldsWithAndrew:
            ...
            # MET pt
            if   shift_syst == self.systNameAK4JetJES+SystNameConvUp:            METToUse['pt_toUse'] = METToUse.T1_pt_jesTotalUp
            elif shift_syst == self.systNameAK4JetJES+SystNameConvDown:          METToUse['pt_toUse'] = METToUse.T1_pt_jesTotalDown
            elif shift_syst == self.systNameAK4JetJER+SystNameConvUp:            METToUse['pt_toUse'] = METToUse.T1_pt_jerUp
            elif shift_syst == self.systNameAK4JetJER+SystNameConvDown:          METToUse['pt_toUse'] = METToUse.T1_pt_jerDown
            elif shift_syst == self.systNameMETUnclE+SystNameConvUp:             METToUse['pt_toUse'] = METToUse.T1_pt_unclustEnUp
            elif shift_syst == self.systNameMETUnclE+SystNameConvDown:           METToUse['pt_toUse'] = METToUse.T1_pt_unclustEnDown
            elif not self.datasetInfo['isMC']:                                   METToUse['pt_toUse'] = METToUse.T1_pt
            else:                                                                METToUse['pt_toUse'] = METToUse.T1_pt               
            # MET phi
            if   shift_syst == self.systNameAK4JetJES+SystNameConvUp:            METToUse['phi_toUse'] = METToUse.T1_phi_jesTotalUp
            elif shift_syst == self.systNameAK4JetJES+SystNameConvDown:          METToUse['phi_toUse'] = METToUse.T1_phi_jesTotalDown
            elif shift_syst == self.systNameAK4JetJER+SystNameConvUp:            METToUse['phi_toUse'] = METToUse.T1_phi_jerUp
            elif shift_syst == self.systNameAK4JetJER+SystNameConvDown:          METToUse['phi_toUse'] = METToUse.T1_phi_jerDown
            elif shift_syst == self.systNameMETUnclE+SystNameConvUp:             METToUse['phi_toUse'] = METToUse.T1_phi_unclustEnUp
            elif shift_syst == self.systNameMETUnclE+SystNameConvDown:           METToUse['phi_toUse'] = METToUse.T1_phi_unclustEnDown
        ...
        calculatePNet_MassH_MassA_cor_and_syst(leadingFatJet)
        ...
        nonHto4bFatJet_PNet_WvsQCD        = array_PutLowerBound(nonHto4bFatJet.particleNet_WvsQCD, kSmallPositiveNumber)
        nonHto4bFatJet_PNet_ZvsQCD        = array_PutLowerBound(nonHto4bFatJet.particleNet_ZvsQCD, kSmallPositiveNumber)
        nonHto4bFatJet_PNet_QCD           = array_PutLowerBound(nonHto4bFatJet.particleNet_QCD,    kSmallPositiveNumber)
        nonHto4bFatJet_PNet_W             = cal_W_(nonHto4bFatJet_PNet_WvsQCD, nonHto4bFatJet_PNet_QCD)
        nonHto4bFatJet_PNet_Z             = cal_Z_(nonHto4bFatJet_PNet_ZvsQCD, nonHto4bFatJet_PNet_QCD)
        nonHto4bFatJet_PNet_WZvsQCD       = cal_WZQ_1_(nonHto4bFatJet_PNet_W, nonHto4bFatJet_PNet_Z, nonHto4bFatJet_PNet_QCD)
        nonHto4bFatJet_PNet_WZvsQCD2      = cal_WZQ_2_(nonHto4bFatJet_PNet_WvsQCD, nonHto4bFatJet_PNet_ZvsQCD) 
        ...
        leadingNonHto4bFatJet = ak.firsts(nonHto4bFatJet[idx_nonHto4bFatJet_WZmax])
        ...
        if self.datasetInfo['isMC']:
            isGenVOvlpVFatJet = ak.any((leadingNonHto4bFatJet.delta_r(genVs) < 0.8), axis=-1)
        ...
        ak4JetsCentral_bTag_nonoverlaping_selFatJets    = ak4Jets_nonoverlaping_selFatJets[(
            (abs(ak4Jets_nonoverlaping_selFatJets.eta) < 2.4) &
            (ak4Jets_nonoverlaping_selFatJets.btagDeepFlavB > self.objectSelector.Ak4JetDeepJetB_Thsh)
        )]
        ...
        # AK4 b jets near Higgs candidate: 0.8 < dR(H, b) < 1.2
        ak4JetsCentral_bTag_near_leadingFatJet = ak4JetsCentral_bTag_nonoverlaping_leadingFatJet[(
            (ak4JetsCentral_bTag_nonoverlaping_leadingFatJet.delta_r(leadingFatJet) > 0.8) & 
            (ak4JetsCentral_bTag_nonoverlaping_leadingFatJet.delta_r(leadingFatJet) < 1.2)
        )]
        nAk4JetsCentral_bTag_near_leadingFatJet   = ak.fill_none(ak.count(ak4JetsCentral_bTag_near_leadingFatJet.eta, axis=1), 0)
        ...
        if ("Haa4bVjjAll_Andrew" in self.sel_conditions_all_list) and ('Haa4b' in events.fields):
            selection.add(
                "Haa4bVjjAll_Andrew", 
                (
                    (events.Haa4b.cat_Vjj == 1) &
                    (events.Haa4b.FatH_tagHaa4b_v1 > 0.975 ) &
                    (events.Haa4b.passFilters == 1) &
                    ((events.Haa4b.trigFat + events.Haa4b.trigBtag) >= 1) &
                    (events.Haa4b.FatH_pt > 250 ) &
                    (events.Haa4b.FatX_tagWZ_max > 0.98 )                     
                )
            )
        ...
        if "candVjj" in self.sel_conditions_all_list:
            selection.add(
                "candVjj",
                (~ak.is_none(leadingNonHto4bFatJet)) # no. of leadingNonHto4bFatJet >= 1
            )
        ...
        if "leadingNonHto4bFatMSoftDrop"  in self.sel_conditions_all_list:
            selection.add(
                ...
                (leadingNonHto4bFatJet.msoftdrop_toUse > self.objectSelector.FatJetMSoftDropThshLow) &
                (leadingNonHto4bFatJet.msoftdrop_toUse < self.objectSelector.FatJetMSoftDropThshHigh)
            )
        if "VFatJetMsoftdrop"  in self.sel_conditions_all_list:
            selection.add(
                "VFatJetMsoftdrop",
                (leadingNonHto4bFatJet.msoftdrop_toUse > self.objectSelector.VFatJetMsoftdrop_Vjj_MinThsh) &
                (leadingNonHto4bFatJet.msoftdrop_toUse < self.objectSelector.VFatJetMsoftdrop_Vjj_MaxThsh)
            )
        ...
        if "leadingFatJetPNet_Xto4bv1_Htoaa4bOverQCD_WP60" in self.sel_conditions_all_list and \
            (('particleNetMD_Hto4b_Haa4b' in FatJetsToUse.fields) or ('PNet_X4b_v1_Haa4b_vs_QCD' in FatJetsToUse.fields) ):
            selection.add(
            ...
            selection.add(
                ...
                ak.fill_none(
                    ~ (
                        ...
                        (leadingNonHto4bTopFatJet.msoftdrop_toUse >  self.objectSelector.TopFatJetMsoftdrop_tt0l_MinThsh) &
                        (leadingNonHto4bTopFatJet.msoftdrop_toUse <  self.objectSelector.TopFatJetMsoftdrop_tt0l_MaxThsh)
                    ),
                    True
                )
            )
        ...
        if "bjetVetoDR" in self.sel_conditions_all_list:
            selection.add(
                "bjetVetoDR",
                nAk4JetsCentral_bTag_near_leadingFatJet == 0
            )
        ...
        if shift_syst is None and printLevel >= 1000:
            for selName, selList in self.sel_names_all.items():
                sel_ = selection.all(* selList )
                print(f"{selName = }, {selList = }")
                printVariable('\n leadingFatJet.pt[sel_]', leadingFatJet.pt[sel_])

        if shift_syst is None and printLevel >= 100:
            selList1_ = ['Haa4bVjjAll_Andrew', 'nPV', 'METFilters', 'candH', 'leadingFatJetPt', 'Trg_Combo_AK4AK8Jet_HT']
            selList2_ = ['PNetWZvsQCDLoose']
            sel1_ = selection.all(* selList1_ )
            sel2_ = selection.all(* selList2_ )
            selTmp_ = sel1_ & (~ sel2_)
            #printVariable('selTmp_', selTmp_)
            printVariable('\nleadingNonHto4bFatJet_PNet_WZvsQCD[selTmp_]', leadingNonHto4bFatJet_PNet_WZvsQCD[selTmp_])           
            printVariable('\n leadingNonHto4bFatJet.particleNet_WZvsQCD[selTmp_]', leadingNonHto4bFatJet.particleNet_WZvsQCD[selTmp_])
            printVariable('\n leadingFatJet.particleNet_WZvsQCD[selTmp_]', leadingFatJet.particleNet_WZvsQCD[selTmp_])
            printVariable('\n nonHto4bFatJet.particleNet_WZvsQCD[selTmp_]', nonHto4bFatJet.particleNet_WZvsQCD[selTmp_])
            #printVariable('\n events.FatJet.particleNet_WZvsQCD[selTmp_]', events.FatJet.particleNet_WZvsQCD[selTmp_])
            for v_ in ['pt', 'eta', 'msoftdrop', 'jetId', 'particleNetMD_XbbvsQCD', 'particleNet_WZvsQCD']:
                printVariable('\n events.FatJet.%s[selTmp_]'%v_, events.FatJet[v_][selTmp_])
            printVariable('\n events.FatJet.Net_X4b_v2ab_Haa34b_score[selTmp_]', (events.FatJet.PNet_X4b_v2a_Haa34b_score + events.FatJet.PNet_X4b_v2b_Haa34b_score)[selTmp_])
            printVariable('\n events.FatJet.delta_r(leadingFatJet)[selTmp_]', events.FatJet.delta_r(leadingFatJet)[selTmp_])
            
        if shift_syst is None and printLevel >= 10:
            selList1_ = ['Haa4bVjjAll_Andrew', 'nPV', 'METFilters', 'candH', 'leadingFatJetPt', 'Trg_Combo_AK4AK8Jet_HT', 'PNetWZvsQCDLoose', "nLeptonsTight", "MetZvvVeto", ]
            selList2_ = ["BJetVeto"]
            sel1_ = selection.all(* selList1_ )
            sel2_ = selection.all(* selList2_ )
            selTmp_ = sel1_ & (~ sel2_)
            ...
            printVariable('\n nonHto4bFatJet.particleNet_WZvsQCD[selTmp_]', nonHto4bFatJet.particleNet_WZvsQCD[selTmp_])
            #printVariable('\n events.FatJet.particleNet_WZvsQCD[selTmp_]', events.FatJet.particleNet_WZvsQCD[selTmp_])
            for v_ in ['pt', 'eta', 'msoftdrop', 'jetId', 'particleNetMD_XbbvsQCD', 'particleNet_WZvsQCD', 'PNet_X4b_v1_Haa4b_vs_QCD']:
                printVariable('\n events.FatJet.%s[selTmp_]'%v_, events.FatJet[v_][selTmp_])
            printVariable('\n events.FatJet.Net_X4b_v2ab_Haa34b_score[selTmp_]', (events.FatJet.PNet_X4b_v2a_Haa34b_score + events.FatJet.PNet_X4b_v2b_Haa34b_score)[selTmp_])
            printVariable('\n events.FatJet.delta_r(leadingFatJet)[selTmp_]', events.FatJet.delta_r(leadingFatJet)[selTmp_])
            printVariable('\n leadingFatJet.PNet_X4b_v1_Haa4b_vs_QCD[selTmp_]', leadingFatJet.PNet_X4b_v1_Haa4b_vs_QCD[selTmp_])
        ...
        if self.datasetInfo["isMC"]:
            ...
            # PNet_WZvsQCD tagger SF
            if self.datasetInfo['isSignalWH'] or self.datasetInfo['isSignalZH']:
                wgt_Ak8PNet_WZvsQCDtag_Nom, wgt_Ak8PNet_WZvsQCDtag_Up, wgt_Ak8PNet_WZvsQCDtag_Down = get_PNet_WZvsQCD_EffiSF(
                    pt                      = leadingNonHto4bFatJet.pt_toUse, 
                    eta                     = leadingNonHto4bFatJet.eta, 
                    year                    = self.datasetInfo["era"], 
                    isGenParticleOvlpFatJet = isGenVOvlpVFatJet
                )
            ...
            if self.datasetInfo['isQCD']:
                wgt_QCDPtRewgt = get_QCDPtWgt(pt_HiggsCandidateAK8Jet = leadingFatJet.pt_toUse)
            ...
            weights.add(
                "lumiWeight",
                weight = copy.deepcopy(lumiScale_toUse)
            )
            weights.add(
                "genWeight",
                weight = copy.deepcopy(np.copysign(np.ones(len(events)), events.genWeight))
            )
            
            if "2018HEM1516Issue" in self.sel_conditions_all_list:
                weights.add(
                    "2018HEM1516IssueWgt",
                    weight = copy.deepcopy(wgt_HEM1516Issue)
                )
            weights.add(
                self.systNamePU,
                weight     = copy.deepcopy(wgt_PU),
            ...
            if self.datasetInfo["era"] != Era_2018:
                weights.add(
                    ...
                    weight     = copy.deepcopy(wgt_L1TPrefiring_dict['Nom']),
            ...
            if self.datasetInfo['isTTbar']:
                weights.add(
                    ...
                    weight     = copy.deepcopy(wgt_TopPt),
            ...
            weights.add(
                self.systNameISR,
                weight     = copy.deepcopy(wgt_PS_Nom),
            ...
            if  kDatasetToAnalyze == DatasetToAnalyze.SingleYear: ## btag 
                weights.add(
                    self.systNameBtag,
                    weight     = copy.deepcopy(wgt_Ak4Btag_dict['Nom']),
                ...
                weights.add(
                    ...
                    weight     = copy.deepcopy(ones_list), #wgt_Ak4Btag_dict['Nom'],  #<<<<<< use dummy weights here to avoid application of btag wgt twice
            ...
            if self.datasetInfo['isQCD']:
                weights.add(
                    "QCDPtRewgt",
                    weight = copy.deepcopy(wgt_QCDPtRewgt)
                )

            if self.datasetInfo['isSignalWH'] or self.datasetInfo['isSignalZH']:
                weights.add(
                    self.systNameAK8JetPNetWZTag,
                    weight     = copy.deepcopy(wgt_Ak8PNet_WZvsQCDtag_Nom),
                    weightUp   = copy.deepcopy(wgt_Ak8PNet_WZvsQCDtag_Up),
                    weightDown = copy.deepcopy(wgt_Ak8PNet_WZvsQCDtag_Down)
                ) 
        ...
        if self.datasetInfo['isMC']:
            if shift_syst is None:
                ...
                if (not self.datasetInfo['systematicsToRun'] == 'no') and self.datasetInfo['runSystematics']:
                    ...
                    if stringHasSubstring(self.datasetInfo['systematicsToRun'], [self.systNameAK8JetPNetWZTag, 'full'] ) and (self.datasetInfo['isSignalWH'] or self.datasetInfo['isSignalZH']):
                        systList.extend( [
                            self.systNameAK8JetPNetWZTag+SystNameConvUp,
                            self.systNameAK8JetPNetWZTag+SystNameConvDown,
                        ] )
        ...
        if shift_syst is None:
            ...
            for iSelection in self.sel_names_all.keys():
                for iCut in range(0, len(self.sel_names_all[iSelection])):
                    iName = f"{iSelection} @ {self.sel_names_all[iSelection][iCut]}: {self.sel_names_all[iSelection][:(iCut+1)]}"
        ...
        if (printLevel >= 0) and (storeIndividualEvtWgts):
            nEvents = len(events)
            print(f"\n\n{ak.sum(weights.weight()) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt'])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=[self.systNameJetTrigEffi])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=[self.systNameAK8JetPNetWZTag])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt', self.systNameJetTrigEffi])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt', self.systNameJetTrigEffi, self.systNameAK8JetPNetWZTag])) / nEvents = }\n\n")
            print(f"\n\n{ak.sum(weights.partial_weight(exclude=['2018HEM1516IssueWgt', self.systNameJetTrigEffi, self.systNameAK8JetPNetWZTag, self.systNameBtagUncorr])) / nEvents = }\n\n")
            ...
            if self.datasetInfo["era"] != Era_2018:
                sWeightsList.extend([self.systNameL1Prefire])
            if self.datasetInfo['isSignal']:
                sWeightsList.extend([self.systNameLPRewgt])
            if self.datasetInfo['isSignalGGH']:
                sWeightsList.extend([self.systNameGGHPtRewgt])
            if self.datasetInfo['isSignalVBFH']:
                sWeightsList.extend([self.systNameVBFHPtRewgt])
            if self.datasetInfo['isSignalWH']:
                sWeightsList.extend([self.systNameWHPtRewgt])
            if self.datasetInfo['isSignalZH']:
                sWeightsList.extend([self.systNameZHPtRewgt])
            if self.datasetInfo['isSignalTTH']:
                sWeightsList.extend([self.systNameTTHPtRewgt])
            if self.datasetInfo['isSignal'] and (not self.datasetInfo['isSignalGGH']) :
                sWeightsList.extend(["HiggsEW_kFactors"])
            if  kDatasetToAnalyze == DatasetToAnalyze.SingleYear:
            ...
            for sWeight in sWeightsList:
                ...
                print(f"\t {sWeight} = {weights.partial_weight(include=[sWeight]).sum() / nEvents},  wgtTotal: {wgtTotal_.sum() / nEvents}")
            print(f"{weights.weight().sum() / nEvents = }, {wgtTotal_.sum() / nEvents = }")

            for sel_name in ["VjjHi", "VjjLo"]:
                wgtTotal_ = np.ones(len(events))
                sel_SR_toUse = selection.all(* self.sel_names_all[sel_name])                
                nEvents      = sel_SR_toUse.sum()
                print(f"\n\n{sel_name = }:: {nEvents = }")
                for sWeight in sWeightsList:
                    wgtTotal_ *= weights.partial_weight(include=[sWeight])
                    print(f"\t {sWeight} = {weights.partial_weight(include=[sWeight])[sel_SR_toUse].sum() / nEvents}")
                print(f"{weights.weight()[sel_SR_toUse].sum() / nEvents = }, {wgtTotal_[sel_SR_toUse].sum() / nEvents = }")
            ...
            for sel_name in self.sel_names_all.keys(): # loop of list of selections
                ...
                for sHExt_0 in self.histosExtensions: # HistogramNameExtensions_QCD = ['_0b', '_1b', '_2b', '_3b', '_4b', '_5bAndMore'], else ['']
                    ...
                    if self.datasetInfo['histogramSaveLevel'] >= 1 and self.datasetInfo['isMC'] and syst == "Nom":
                        ...
                        if self.datasetInfo['isSignalTTH']:
                            output['hEventWeight_TTHHiggsPt'+sHExt].fill(
                                dataset=dataset,
                                Weight=wgt_TTHaa_HiggsPt[sel_SR_forHExt]
                            )
                            output['hEventWeight_TTHHiggsPtUp'+sHExt].fill(
                                dataset=dataset,
                                Weight=wgt_TTHaa_HiggsPtUp[sel_SR_forHExt]
                            )
                            output['hEventWeight_TTHHiggsPtDown'+sHExt].fill(
                                dataset=dataset,
                                Weight=wgt_TTHaa_HiggsPtDown[sel_SR_forHExt]
                            )
                        if self.datasetInfo['isSignal'] and (not self.datasetInfo['isSignalGGH']) :
                            output['hEventWeight_HiggsEW'+sHExt].fill(
                                dataset=dataset,
                                Weight=EWcorr[sel_SR_forHExt]
                            )
                    ...
                    if self.datasetInfo['histogramSaveLevel'] >= 0: 
                        ...
                        output['hLeadingFatJetPhi_EtaLtm1p1'+sHExt].fill(
                            dataset=dataset,
                            Phi=(leadingFatJet.phi[sel_SR_forHExt & ak.fill_none((leadingFatJet.eta < -1.1), False)]),
                            systematic=syst,
                            weight=evtWeight[sel_SR_forHExt & ak.fill_none((leadingFatJet.eta < -1.1), False)]
                        )
                        ...
                        output['hLeadingNonHto4bFatJetPhi_EtaLtm1p1'+sHExt].fill(
                            dataset=dataset,
                            Phi=(leadingNonHto4bFatJet.phi[sel_SR_forHExt & ak.fill_none((leadingNonHto4bFatJet.eta < -1.1), False)]),
                            systematic=syst,
                            weight=evtWeight[sel_SR_forHExt & ak.fill_none((leadingNonHto4bFatJet.eta < -1.1), False)]
                        )
                        ...
                        output['hLeadingNonHto4bTopFatJetPNet_TvsQCD'+sHExt].fill(
                            dataset=dataset,
                            MLScore1k=(leadingNonHto4bTopFatJet_PNet_TvsQCD[sel_SR_forHExt]),
                            systematic=syst,
                            weight=evtWeight[sel_SR_forHExt]
                        )
