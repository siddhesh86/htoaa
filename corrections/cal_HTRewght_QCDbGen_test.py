...
if __name__ == "__main__":
    ...
    sFOut_dict = {
        Era_2016: "",
        Era_2017: "",        
        Era_2018: "../data/correction/mc/HTSamplesStitch/HTSamplesStitchSF_2018_tmp.root"
    }
    ...
    fIn  = uproot.open(sFIn)
    fOut = uproot.recreate(sFOut)
    ...
    for sample_category in sHistogramNameForSF:
        ...
        print(f" {hN_.view() = } ")
        print(f" { list( hN_.values() )  = } ")
        print(f" { hN_.values().shape  = } ")

        
        h1 = hN_

        
        for iBin in range(1, hN_.values().shape[0]):
            h1[iBin] = 1 

        print(f"\n\n\n {h1.view() = } ")
        print(f" { list( h1.values() )  = } ")
        print(f" { h1.values().shape  = } ")
