...
SampleName=${prodmode} 
outputFileStep1="${outputFile%.root}_step1.root"

# Dump actual test code to a DRPremix_${SampleName}_test.sh file that can be run in Singularity
cat << EndOfTestFile > DRPremix_${SampleName}_test.sh
#!/bin/bash

export SCRAM_ARCH=el8_amd64_gcc12

source /cvmfs/cms.cern.ch/cmsset_default.sh
echo "Running DRPremix_${SampleName}_test.sh "
echo "pwd: "
pwd
echo "ls -ltrh: "
ls -ltrh 
...
REPORT_NAME=DRPremix_${SampleName}_0_report.xml
# Run the cmsRun
cmsRun -e -j \$REPORT_NAME DRPremix_${SampleName}_1_cfg.py || exit \$? ;

printf "\n\ncmsRun 1st: done \n\n"
# Parse values from DRPremix_${SampleName}_0_report.xml report
...
echo "Validation report of DRPremix_${SampleName} sequence 1/2"
...
echo "cmsDriver.py 2nd"

# cmsDriver command
cmsDriver.py  --era Run3_2024 --customise Configuration/DataProcessing/Utils.addMonitoring --step RAW2DIGI,L1Reco,RECO,RECOSIM --geometry DB:Extended --conditions 140X_mcRun3_2024_realistic_v26 --datatier AODSIM --eventcontent AODSIM --python_filename DRPremix_${SampleName}_2_cfg.py --fileout file:${outputFile} --filein file:${outputFileStep1} --number \${EVENTS} --number_out \${EVENTS} --no_exec --mc || exit \$? ;
...
printf "\n\ncmsRun 2nd: \n\n"
# Run generated config
REPORT_NAME=DRPremix_${SampleName}_report.xml
# Run the cmsRun
cmsRun -e -j \$REPORT_NAME DRPremix_${SampleName}_2_cfg.py || exit \$? ;

printf "\n\ncmsRun 2nd: done \n\n" 
# Parse values from DRPremix_${SampleName}_report.xml report
processedEvents=\$(grep -Po "(?<=<Metric Name=\"NumberEvents\" Value=\")(.*)(?=\"/>)" \$REPORT_NAME | tail -n 1)
producedEvents=\$(grep -Po "(?<=<TotalEvents>)(\d*)(?=</TotalEvents>)" \$REPORT_NAME | tail -n 1)
threads=\$(grep -Po "(?<=<Metric Name=\"NumberOfThreads\" Value=\")(.*)(?=\"/>)" \$REPORT_NAME | tail -n 1)
peakValueRss=\$(grep -Po "(?<=<Metric Name=\"PeakValueRss\" Value=\")(.*)(?=\"/>)" \$REPORT_NAME | tail -n 1)
...
echo "Validation report of DRPremix_${SampleName} sequence 2/2"
echo "Processed events: \$processedEvents"
echo "Produced events: \$producedEvents"
echo "Threads: \$threads"
echo "Peak value RSS: \$peakValueRss MB"
...
# End of DRPremix_${SampleName}_test.sh file
EndOfTestFile

printf "\n\n cat DRPremix_${SampleName}_test.sh  \n"
cat DRPremix_${SampleName}_test.sh
printf "\n\n"

# Make file executable
chmod +x DRPremix_${SampleName}_test.sh
...
singularity run --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/DRPremix_${SampleName}_test.sh)
